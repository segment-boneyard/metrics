...
Metric.prototype.from = function (date, window) {
  ...
  if (!isNumber(window)) {
    ...
    else window = ms ('1 hour');
  }

  // sort the dates
  var dates = sortBy(this.timestamps(), function (timestamp) {
    // calculate the distance
    return Math.abs(date.getTime() - timestamp.getTime()); 
  });

  if (dates.length > 0 && // if we we have metric values available
      Math.abs(dates[0].getTime() - date.getTime()) <= window) { // and they are close enough to desired window
    return this._values[dates[0].getTime()];
  } else {
    return null;
  }
};